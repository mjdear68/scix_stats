# Inferential Statistics {#inferential_stats}
...
The boxplot for the [PlantGrowth data set](./resources/datasets/PlantGrowth.csv) below suggests there is a difference in the mean between the trt1 and trt2 groups, but how can we test this hypotheses in a quantitative way? This is the purpose of statistical tests. 
...
Reproduce the hypothesis tests for the [PlantGrowth data set](./resources/datasets/PlantGrowth.csv). Use both [Statistics Kingdom](https://www.statskingdom.com/index.html) and Excel. Compare the results.

Note: You may need to [activate the Analysis ToolPak in Excel](https://support.microsoft.com/en-au/office/use-the-analysis-toolpak-to-perform-complex-data-analysis-6c67ccf0-f4a9-487c-8dec-bdb5a2cefab6) to complete statistical tests.
...
* the sample is drawn from a [normally distributed ](https://www.abs.gov.au/statistics/understanding-statistics/statistical-terms-and-concepts/measures-shape) population
...
PlantGrowth %>% 
  ...
  stat_qq_line() +
  facet_wrap(.~group) +
  labs(title = "Q-Q Plot For Each Group", y = "Weight")

PlantGrowth %>% 
  group_by(group) %>% 
  summarise(p = shapiro.test(weight)$p.value) %>% 
  gt() %>% 
  tab_header(
    title = md("PlantGrowth Shapiro-Wilk Test For Normality" ),
    subtitle = md("p-values of `weight` for each group")
   )

```


## Common Statistical Tests {#common_stat_tests}
...
[**Statistical Test Finder**](https://www.statskingdom.com/statistical-decision-tree.html), Statistics Kingdom

[**Shapiro-Wilk Calculator**](https://www.statskingdom.com/shapiro-wilk-test-calculator.html), Statistics Kingdom


## Further Reading

[**Estimation**](https://stats.libretexts.org/Bookshelves/Introductory_Statistics/Introductory_Statistics_(Shafer_and_Zhang)/07%3A_Estimation), Shafer and Zhang

[**Confidence Intervals**](https://online.stat.psu.edu/stat200/lesson/4), Pennsylvania State University

[**Estimating a Confidence Interval**](https://stats.libretexts.org/Bookshelves/Applied_Statistics/Learning_Statistics_with_R_-_A_tutorial_for_Psychology_Students_and_other_Beginners_(Navarro)/10%3A_Estimating_Unknown_Quantities_from_a_Sample/10.05%3A_Estimating_a_Confidence_Interval), Navarro D
