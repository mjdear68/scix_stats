...
# Introduction {#descriptive_stats}
...
For data sets with grouping variables, it is also worthwhile creating a summary for each group. 
```{r warning=FALSE}
vtable::st(iris,
           summ = list(c('notNA(x)', 'countNA(x)', 'mean(x)',   'sd(x)', 'min(x)', 'pctile(x)[25]', 'median(x)', 'pctile(x)[75]', 'max(x)')),
           summ.names = list(c("notNA", "NA", "Mean", "SD", "Min", "Q1", "Q2 (Median)", "Q3", "Max")),
...
Line plots are used for time series data e.g. daily temperatures in March.
...
1. Use the [PlantGrowth](resources/datasets/PlantGrowth.csv) data set and a spreadsheet to

  a. Create a summary of the quantitative variable for each group including n, min, max, quantiles, mean, standard deviation 
  b. Create boxplots of the quantitative variable for each group in the data set. 
  c. Create histograms of the quantitative variable for each group in the data set.
  d. Write a summary of your observations. Can you suggest any hypotheses?
...
2. Repeat the steps of Q.1 using the following two online resources
  a. [**Statistics Kingdom Descriptive Statistics Calculator**](https://www.statskingdom.com/descriptive-statistics-calculator.html)
  b. [**HHMI Data Explorer**](media.hhmi.org/biointeractive/tool/dataexplorer/v1/#/home)
  
3. Complete a table of pros and cons for each tool.
