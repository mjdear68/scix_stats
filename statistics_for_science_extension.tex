...
The output above shows that the Iris data set has five variables, four that are continuous \emph{(dbl)} and one that is qualitative \emph{(fct)}. There are 150 records (rows) in the data set.

The following is a summary of descriptive statistics for each variable in the Iris data set.

\begin{verbatim}
Warning in vtable::st(iris, summ = list(c("notNA(x)", "countNA(x)", "mean(x)", : Factor variables ignore custom summ options. Cols 1 and 2 are count and percentage.
